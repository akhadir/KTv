...
app.get('/getQueuedList', function (req, res) {
    queReqHandler = res;
    getQueuedList(res);
});
function getQueuedList (res) {
    if (queuedList) {
          res.send(JSON.stringify(queuedList));
          queuedList = null;
    } else {
        if (ss) {
            ss.emit("controlling", "player-qlist");
        } else {
            res.send([]);
        }
    }
...
io.sockets.on('connection', function (socket) {
    ...
    socket.on("reverse-controll", function (data) {
        queuedList = data;
        getQueuedList(queReqHandler);
    });
    socket.on("controll", function (data) {
        if (socket.type === "remote") {
            ...
            if (data.action === "tap") {
                ...
                runShell("/usr/bin/lxterminal", ['-e', '/home/pi/fullscree.sh'], {
                        DISPLAY: ':0.0'
                    }, function (me, buffer) {
            ...
            } else if (data.action === "tvrestart") {
                console.log("Command: Browser restart");
                runShell("/usr/bin/lxterminal", ['-e', '/home/pi/restartPir.sh'], {
                        DISPLAY: ':0.0'
                    }, function (me, buffer) {
                        console.log(me.stdout + buffer.toString());
                    },
